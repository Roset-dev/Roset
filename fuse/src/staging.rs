use crate::client::{RosetClient, Part};
use anyhow::Result;
...
impl StagingManager {
    pub fn new(client: Arc<RosetClient>, staging_root: PathBuf) -> Self {
        ...
        if !failed_dir.exists() {
            std::fs::create_dir_all(&failed_dir).expect("Failed to create staging failed directory");
        }
                                    ...
                                    match serde_json::from_slice::<UploadJob>(&content) {
                                        Ok(job) => {
                                            if job.file_path.exists() {
                                                ...
                                                warn!("Orphaned job file (data missing): {:?}", path);
                                        ...
                                        Err(e) => {
                                            ...
                                            // Move to failed
                                            let dest = root_clone.join("failed").join(path.file_name().unwrap());
        ...
        tokio::spawn(async move {
            ...
            while let Some(mut job) = rx.recv().await {
                    ...
                    match process_upload(&client, &mut job).await {
                        Ok(_) => {
                            ...
                            if let Err(e) = fs::remove_file(&job.file_path).await {
                                error!("Failed to remove staged file {}: {}", job.file_path.display(), e);
                            }
                            // Cleanup job file
                            let job_file_path = job.file_path.with_extension("job.json");
                             if let Err(e) = fs::remove_file(&job_file_path).await {
                                ...
                                if e.kind() != std::io::ErrorKind::NotFound {
                                     error!("Failed to remove job file {}: {}", job_file_path.display(), e);
                                }
                            }
                            success = true;
                            break;
                        ...
                        Err(e) => {
                            attempts += 1;
                            error!("Upload attempt {}/{} failed for {}: {}", attempts, max_attempts, job_info, e);
                ...
                if !success {
                    error!("FATAL: Failed to upload {} after {} attempts. Moving to DLQ.", job_info, max_attempts);
                    ...
                    // Move job to failed/
                    let job_file_path = job.file_path.with_extension("job.json");
                    let job_failed_dest = root.join("failed").join(job_file_path.file_name().unwrap());
    ...
    pub async fn stage_file(&self, source_path: &Path, node_id: String, upload_token: String, size: u64) -> Result<()> {
        ...
        self.sender.send(job).await.map_err(|e| anyhow::anyhow!("Failed into enqueue upload job: {}", e))?;

...
<CodeBlock
  language="bash"
  code={`pip install roset[all]  # All frameworks
# or
...
client = Client(api*url="https://api.roset.dev", api_key="rsk*...")
...
<CodeBlock language="python" code={`from transformers import Trainer
from roset import Client
...
<CodeBlock language="python" code={`# 1. Create a commit group
...
<CodeBlock language="python" code={`# Get current "latest" pointer

...
**Path resolution is cached** with a 5-minute TTL. After rename/move, cached paths may be stale briefly.
...
Renaming a file or folder is **O(1)**. Moving a folder updates the path for all its descendants (Materialized Path pattern). This trade-off enables blazing fast O(1) path resolution and subtree querying.

```http
PATCH /v1/nodes/:id { name: "new-name", parentId: "new-parent-id" }
```

### O(1) Hierarchical Queries

Thanks to materialized paths, querying ancestors or all descendants of a folder is **O(1)** (index scan), without requiring recursive database operations.
...
- Path → Node resolution is cached with 5-minute TTL

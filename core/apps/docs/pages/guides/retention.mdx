...
console.log(node.deletedAt); // "2024-01-15T10:30:00Z"
```
...
```typescript
await client.nodes.restore(nodeId);
```
...
```typescript
const { retention } = await client.mounts.get(mountId);

console.log(retention.retentionDays); // 30
console.log(retention.trashCount); // 42 files in trash
console.log(retention.oldestTrash); // "2024-01-01T00:00:00Z"
```

...
<CodeBlock language="typescript" code={`const mounts = await client.mounts.list();
...
<CodeBlock
  language="typescript"
  code={`const mount = await client.mounts.create({
  name: "eu-storage",
  provider: "s3",
  bucket: "my-app-eu",
  region: "eu-west-1",
...
<CodeBlock language="typescript" code={`const { mount, retention } = await client.mounts.get(mountId);
...
<CodeBlock
  language="typescript"
  code={`await client.mounts.delete(mountId);`}
/>
...
<CodeBlock language="typescript" code={`// Default mount
...
<CodeBlock
  language="typescript"
  code={`const defaultMount = await client.mounts.getDefault();`}
/>

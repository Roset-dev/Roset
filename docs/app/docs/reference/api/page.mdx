...
| `variants` | string[] | No | Variant types to generate (e.g., `["markdown", "embeddings"]`). Defaults to all applicable types. |
...
Common variant types: `markdown`, `embeddings`, `metadata`, `searchable-index`.
...
```json
{
  ...
  "default_variants": ["markdown", "embeddings", "metadata", "searchable-index"]
}
```

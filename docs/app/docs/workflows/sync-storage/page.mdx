...
<PromptCard
  ...
  prompt="Roset (roset.dev) is a developer API that transforms unstructured files into structured data (markdown, embeddings, metadata, searchable index). It connects to cloud storage (S3, GCS, Azure Blob) and processes files in-place. Supported file types: PDFs, images, audio, DOCX. API endpoints: POST /v1/connections to connect a bucket, POST /v1/connections/:id/sync to sync files, GET /v1/files to list synced files, POST /v1/upload to process a file. Webhooks notify on file.completed. Write me a guide for connecting an S3 bucket to Roset via the API, syncing files, and setting up a webhook for completion notifications."
/>

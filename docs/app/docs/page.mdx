...
export const metadata = {
  ...
  description: 'The transformation engine for unstructured data. Upload any document — get back markdown, embeddings, metadata, thumbnails, and a searchable index.',
}
...
- **Provider Keys** are the API credentials for extraction providers (Reducto, OpenAI, Gemini, Whisper). All tiers use BYOK (Bring Your Own Keys). A small managed trial quota is included for evaluation — after the trial, configure your own provider keys in Settings.

...
export const metadata = {
  ...
  description: 'Transform unstructured files into structured data. Upload any document -- get back markdown, embeddings, metadata, thumbnails, and a searchable index.',
}
...
Roset is the **transformation engine for unstructured data**. Upload any file -- PDF, image, audio, document -- and Roset routes it to the right extraction provider, generates embeddings, and gives you back five structured variants: **markdown**, **embeddings**, **metadata**, **thumbnails**, and a **searchable index**.
...
- **Variants** are the extraction outputs linked to a parent file -- extracted markdown, vector embeddings, thumbnails, or structured metadata. See [Variant Types](/docs/features/variants) for details.
